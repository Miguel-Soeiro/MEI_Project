...
dinic <- c()
for (i in x) {
    file <- sprintf("alg/prob2/%d/Dinic_%d.txt", i, i)
...
ek <- c()
for (i in x) {
    file <- sprintf("alg/prob2/%d/EK_%d.txt", i, i)
    conn <- file(file, open="rb")
    lines <- read_lines(conn, n_max = 80)
    times <- as.numeric(lines[seq(2, length(lines), 2)])
    avg_time <- mean(times) * 1000000 # Seconds to microseconds
...
mpm <- c()
for (i in x) {
    file <- sprintf("alg/prob2/%d/MPM_%d.txt", i, i)
    conn <- file(file, open="rb")
    lines <- read_lines(conn, n_max = 80)
    times <- as.numeric(lines[seq(2, length(lines), 2)])
    avg_time <- mean(times) * 1000000 # Seconds to microseconds
...
# Make a empty plot
plot(x, dinic, ylim = c(0, 200), xlab = "Number of Vertices", ylab = "Time (microseconds)")
