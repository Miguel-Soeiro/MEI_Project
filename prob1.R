...
x <- c(150, 200, 250, 300, 350, 400, 450, 500)
...
dinic <- c()
for (i in x) {
  file <- sprintf("alg/prob1/%d/Dinic_%d.txt", i, i)
...
ek <- c()
for (i in x) {
  file <- sprintf("alg/prob1/%d/EK_%d.txt", i, i)
  conn <- file(file, open="rb")
  lines <- read_lines(conn, n_max = 80)
  times <- as.numeric(lines[seq(2, length(lines), 2)])
  avg_time <- mean(times) * 1000000 # Seconds to microseconds
...
mpm <- c()
for (i in x) {
  file <- sprintf("alg/prob1/%d/MPM_%d.txt", i, i)
  conn <- file(file, open="rb")
  lines <- read_lines(conn, n_max = 80)
  times <- as.numeric(lines[seq(2, length(lines), 2)])
  avg_time <- mean(times) * 1000000 # Seconds to microseconds
...
dinic_lr <- lm(sqrt(dinic) ~ x)
ek_lr <- lm(sqrt(ek) ~ x)
mpm_lr <- lm(sqrt(mpm) ~ x)

summary(ek_lr)
summary(mpm_lr)
summary(dinic_lr)

dinic_lr <- lm(1/dinic ~ x)
ek_lr <- lm(1/ek ~ x)
mpm_lr <- lm(1/mpm ~ x)
...
# Make a empty plot
plot(x, dinic, ylim = c(0, 1000000), xlab = "Number of Vertices", ylab = "Time (microseconds)")
